...
import { addMotorData, deleteMotorData, getMotorData } from "../../../actions/motorExaminationAction";
import { connect } from "react-redux";
import EditMotor from './edit/motor'
//icons
...
function Motor(props) {
    ...
    React.useEffect(() => {
        if (props.id == undefined) props.getMotorData(props.iid);
        else props.getMotorData(props.id);
    }, []);

    const [data, setData] = React.useState({});
    const [openEditMotor, setOpenEditMotor] = React.useState(false);
    ...
    const onSubmit = (formValues) => {
        ...
        if (props.id == undefined) {
            data.patient = props.iid;
        }
        else {
            data.patient = props.id;
        }
        console.log(data);
    ...
    const handleClose = () => {
        setOpenEditMotor(false);
    }

    const motorTableHeading = [
        { id: "date", label: "Date" },
        { id: 'fields', label: 'Fields which are not empty' },
        { id: "actions", label: "Actions" }
    ];

    const deleteMotorHandler = (id) => {
        props.deleteMotorData(id);
    }
    console.log(props)

    const motorData = [];
    props.motorData &&
        props.motorData.forEach((data, index) => {
            const datas = {
                date: `${data.date}`,
                fields: `d`,
                actions: (
                                    ...
                                    <thead>
                                        <tr>
                                            <th>
                                                ...
                                                >Combined Movement Assesment Of Spine</h5>
                                            </th>
                                        </tr>
                                    </thead>
                                    {open === 14 && (
